...
test("demo snapshot covers the complete SPIFFE mTLS flow", () => {
  const snapshot = getDemoSnapshot("127.0.0.1", 5173);

  assert.equal(snapshot.commands.web, "pnpm demo:web");
...
test("demo page embeds the interactive model", () => {
  const html = renderDemoPage(getDemoSnapshot("127.0.0.1", 5173));

  assert.match(html, /SPIFFE mTLS Agent 过程演示/);
  assert.match(html, /window\.__DEMO__/);
  assert.match(html, /播放成功路径/);
  assert.match(html, /\/assets\/spiffe-agent-mtls-complete-architecture\.svg/);
});
